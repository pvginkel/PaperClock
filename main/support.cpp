...
int get_dst_offset(time_t time) {
    auto offset = dst_transitions[0].offset;

    for (int i = 0; i < sizeof(dst_transitions); i++) {
        if (dst_transitions[i].time > time) {
            return offset;
        }
        offset = dst_transitions[i].offset;
    }

    ESP_LOGW(TAG, "Could not get DST offset for time %" PRIu32, (uint32_t)time);
